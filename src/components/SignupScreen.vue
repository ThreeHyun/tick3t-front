  ...
  <div>
    ...
    <v-card class="mx-auto pa-12 pb-4" elevation="8" max-width="530" rounded="lg">
      ...
      <v-text-field v-model="email" density="compact" prepend-inner-icon="mdi-email-outline" variant="outlined"
        placeholder="예) tick3t@tick-3t.com" label="이메일" type="email" required @blur="emailValid"></v-text-field>
      ...
      <v-text-field v-model="userPwd" :type="visible ? 'text' : 'password'" density="compact" label="비밀번호"
        prepend-inner-icon="mdi-lock-outline" variant="outlined" maxlength="16" @blur="userPwdValid"
      ...
      <v-text-field v-model="userPwdCheck" :type="visible ? 'text' : 'password'" density="compact"
        prepend-inner-icon="mdi-lock-outline" variant="outlined" label="비밀번호 확인" maxlength="16" @blur="userPwdCheckValid"
        required></v-text-field>
      ...
      <v-btn @click="sendSignup" block class="text-white mb-8" color="primary" size="large" variant="flat"
        :disabled="email && userPwd && userPwdCheck && name && birth && fanCd === ''">
        가입완료
      </v-btn>
    </v-card>
  </div>
...
export default {
  ...
  methods: {
    ...
    emailValid() {
      if (/[0-9a-zA-Z]([-_.]?[0-9a-zA-Z])*@[0-9a-zA-Z]([-_.]?[0-9a-zA-Z])*.[a-zA-Z]$/.test(this.email)) {
